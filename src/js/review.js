var per = [
    { ID: '暂无审核信息', creator: '暂无审核信息', operation: '暂无可执行操作' },
];

function LoadNewsTable() {
    document.getElementsByTagName("h5")[0].innerHTML = "待审核新闻页";
    $.ajax({
        url: 'https://lsservice.leicacloud.com:1201/api/html/getReviewNews',
        type: 'GET',
        data: '',
        dataType: "json",
...
function LoadProductTable() {
    document.getElementsByTagName("h5")[0].innerHTML = "待审核产品页";
    $.ajax({
        url: 'https://lsservice.leicacloud.com:1201/api/html/getReviewProduct',
        type: 'GET',
        data: '',
        dataType: "json",
        contentType: "application/json;utf-8",
...
function LoadNewsReviewTable() {
    document.getElementsByTagName("h5")[0].innerHTML = "已审核新闻页";
    $.ajax({
        url: 'https://lsservice.leicacloud.com:1201/api/html/getReviewedNews',
        type: 'GET',
        data: '',
        dataType: "json",
        contentType: "application/json;utf-8",
...
function LoadProductReviewTable() {
    document.getElementsByTagName("h5")[0].innerHTML = "已审核产品页";
    $.ajax({
        url: 'https://lsservice.leicacloud.com:1201/api/html/getReviewedProduct',
        type: 'GET',
        data: '',
        dataType: "json",
        contentType: "application/json;utf-8",
...
function getDataRow(h) {
    ...
    var btnReview = document.createElement('input'); //创建一个input控件 
    btnReview.setAttribute('type', 'button'); //type="button" 
    btnReview.setAttribute('value', '通过审核');
    btnReview.onclick = function () {//审核操作 
        if (confirm("确定审核通过吗？")) {
            var sendinfo = {}
            sendinfo["id"] = h.ID;
            $.ajax({
                url: 'https://lsservice.leicacloud.com:1201/api/html/reviewPass',
                type: 'POST',
                ...
                success: function (data) {
                    if (data['Status'] == '200') {
                        ...
                        if (document.getElementsByTagName("h5")[0].innerHTML == "待审核产品页") {
                            LoadProductTable();
                        } else {
                            LoadNewsTable();
                        }
    ...
    var btnReject = document.createElement('input'); //创建一个input控件 
    btnReject.setAttribute('type', 'button'); //type="button" 
    btnReject.setAttribute('value', '驳回');
    btnReject.onclick = function () {//审核操作 
        if (confirm("确定驳回审核申请吗？")) {
            var sendinfo = {}
            sendinfo["id"] = h.ID;
            $.ajax({
                url: 'https://lsservice.leicacloud.com:1201/api/html/reviewReject',
                type: 'POST',
                data: JSON.stringify(sendinfo),
                dataType: "json",
                contentType: "application/json;utf-8",
    ...
    var btnRereview = document.createElement('input'); //创建一个input控件 
    btnRereview.setAttribute('type', 'button'); //type="button" 
    btnRereview.setAttribute('value', '再次审核');
    btnRereview.onclick = function () {//审核操作 
        if (confirm("确定重新审核吗？")) {
            var sendinfo = {}
            sendinfo["id"] = h.ID;
            $.ajax({
                url: 'https://lsservice.leicacloud.com:1201/api/html/reviewRe',
                type: 'POST',
                data: JSON.stringify(sendinfo),
                dataType: "json",
                contentType: "application/json;utf-8",
                success: function (data) {
                    if (data['Status'] == '200') {
                        console.log(data['Msg']);
                        if (document.getElementsByTagName("h5")[0].innerHTML == "已审核产品页") {
                            LoadProductReviewTable();
                        } else {
                            LoadNewsReviewTable();
                        }
                    }
    ...
    if (h.review_status == "wait") {
        ...
        reviewCell.appendChild(btnReview); //把审核按钮加入td
        reviewCell.appendChild(btnReject); //把审核按钮加入td，别忘了 
    } else if (h.review_status == "pass" || h.review_status == "reject") {
        reviewCell.appendChild(btnLoad);
        reviewCell.appendChild(btnRereview); //把重新审核按钮加入td
